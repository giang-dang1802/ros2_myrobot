...
hardware_interface::CallbackReturn STM32DiffDriveHardware::on_activate(const rclcpp_lifecycle::State &)
{
    ...
    pending_lift_cmd_ = "";
    lift_node_ = rclcpp::Node::make_shared("lift_bridge_internal");
    lift_sub_ = lift_node_->create_subscription<std_msgs::msg::String>(
        "/lift_cmd",
        10,
        [this](const std_msgs::msg::String::SharedPtr msg) {
            std::lock_guard<std::mutex> lock(lift_mutex_);
            pending_lift_cmd_ = msg->data;
            RCLCPP_INFO(rclcpp::get_logger("STM32Hardware"),
                "Nhận lệnh LIFT: %s", msg->data.c_str());
        }
    );
    lift_thread_ = std::thread([this]() {
        rclcpp::spin(lift_node_);
    });
...
{
    // ✅ Dừng lift_node_ đúng cách
    if (lift_node_) {
        lift_node_->get_node_base_interface()->get_context()->shutdown("deactivate");
    }

    // ✅ Chờ thread kết thúc
    if (lift_thread_.joinable()) {
        lift_thread_.join();
    }

    // ✅ Đóng serial 1 lần duy nhất
    if (serial_.is_open()) {
        serial_.close();
    }
...
hardware_interface::return_type STM32DiffDriveHardware::write(const rclcpp::Time &, const rclcpp::Duration &)
{
    ...
    {
        std::lock_guard<std::mutex> lock(lift_mutex_);
        if (!pending_lift_cmd_.empty()) {
            std::string lift_tx = pending_lift_cmd_ + "\n";
            try {
                boost::asio::write(serial_, boost::asio::buffer(lift_tx));
                RCLCPP_INFO(rclcpp::get_logger("STM32"),
                    "TX LIFT: '%s'", lift_tx.c_str());
            } catch (...) {}
            pending_lift_cmd_ = "";  // Xóa sau khi gửi
        }
    }
    ...
    cmd_left_rad_per_s  = std::clamp(cmd_left_rad_per_s,  -50.0f, 50.0f); 
    cmd_right_rad_per_s = std::clamp(cmd_right_rad_per_s, -50.0f, 50.0f);
